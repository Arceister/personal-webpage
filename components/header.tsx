    ...
    <nav className="flex justify-center lg:justify-between flex-wrap sm:p-3 lg:p-5">
      <div className="w-full block">
        <div className="flex justify-around sm:text-xl lg:text-2xl">
          <Link 
            href='/' 
            className={`block mt-4 lg:inline-block lg:mt-0 text-slate-800 hover:text-black mr-4 ${activeCondition(0)}`} 
            onClick={() => {setActiveIndicator(0)}}>
            Home
          </Link>
          <Link href='/' 
            className={`block mt-4 lg:inline-block lg:mt-0 text-slate-800 hover:text-black mr-4 ${activeCondition(1)}`}
            onClick={() => {setActiveIndicator(1)}}>
            Blog
          </Link>
          <Link href='/' 
            className={`block mt-4 lg:inline-block lg:mt-0 text-slate-800 hover:text-black mr-4 ${activeCondition(2)}`}
